...
const defaultPhaseDeliverables = (phase: 1 | 2 | 3) => {
  if (phase === 1) {
    return [
      { id: 'p1-1', label: '140 Scientific References Received & Validated', completed: true },
...
export const ERROR_CATALOG: ErrorCatalogEntry[] = [
  ...
  },
  {
    id: 'EC-006',
    code: 'CRF-DUP-001',
    shortMessage: 'Duplicate CRF variable detected',
    detailedMessage: 'Two or more CRF fields use the same normalized label and may collect the same clinical construct twice.',
    category: 'crf_structural',
    severity: 'critical',
    suggestedCorrection: 'Merge duplicate fields or rename them with distinct operational definitions.',
    supportingReferenceIds: ['CDISC_CDISC'],
    applicableStudyTypes: ['*'],
  },
  {
    id: 'EC-007',
    code: 'CRF-CONF-001',
    shortMessage: 'Conflicting variable definitions',
    detailedMessage: 'Variables appear to measure the same construct but use different units, scales, or collection methods.',
    category: 'crf_structural',
    severity: 'high',
    suggestedCorrection: 'Choose one authoritative definition or document separate constructs with explicit units and timing.',
    supportingReferenceIds: ['CDISC_CDISC'],
    applicableStudyTypes: ['*'],
  },
  {
    id: 'EC-008',
    code: 'VAR-LINK-001',
    shortMessage: 'Outcome not linked to research question',
    detailedMessage: 'Primary and secondary outcomes must be explicitly traceable to the research question they answer.',
    category: 'methodological',
    severity: 'critical',
    suggestedCorrection: 'Link each outcome variable to at least one research question in the Variable Matrix.',
    supportingReferenceIds: ['SPIRIT_2013', 'STROBE_STATEMENT', 'PRISMA_2020'],
    applicableStudyTypes: ['*'],
  },
  {
    id: 'EC-009',
    code: 'VAR-LINK-002',
    shortMessage: 'Outcome not linked to objective',
    detailedMessage: 'Outcome variables without objective linkage weaken traceability from protocol aims to CRF and SAP.',
    category: 'methodological',
    severity: 'high',
    suggestedCorrection: 'Link each outcome to a primary or secondary objective.',
    supportingReferenceIds: ['SPIRIT_2013', 'PRISMA_P_2015'],
    applicableStudyTypes: ['*'],
  },
  {
    id: 'EC-010',
    code: 'VAR-META-001',
    shortMessage: 'Variable metadata incomplete',
    detailedMessage: 'A variable is missing source, measurement method, unit, scale, or recommended statistical test metadata.',
    category: 'statistical',
    severity: 'high',
    suggestedCorrection: 'Complete the Variable Matrix metadata before exporting the CRF or SAP.',
    supportingReferenceIds: ['CDISC_CDISC', 'ICH_E9'],
    applicableStudyTypes: ['*'],
  },
  {
    id: 'EC-011',
    code: 'SAP-COMP-001',
    shortMessage: 'Variable test not listed in SAP',
    detailedMessage: 'A variable recommends a statistical test that is not present in the planned Statistical Analysis Plan.',
    category: 'statistical',
    severity: 'moderate',
    suggestedCorrection: 'Add the test to the SAP or update the variable-level recommended test.',
    supportingReferenceIds: ['ICH_E9', 'ICH_E9_R1'],
    applicableStudyTypes: ['rct', 'prospective', 'retrospective', 'cross_sectional', 'in_vitro'],
  },
  {
    id: 'EC-012',
    code: 'RAG-ASSET-001',
    shortMessage: 'Reference PDF/text asset missing',
    detailedMessage: 'The catalog contains reference metadata, but the local source artifact is not linked in the manifest.',
    category: 'reference_scope',
    severity: 'moderate',
    suggestedCorrection: 'Add the PDF/text/HTML artifact and regenerate reference_sources_manifest.json.',
    supportingReferenceIds: ['EQUATOR_NETWORK'],
    applicableStudyTypes: ['*'],
  },
  {
    id: 'EC-013',
    code: 'SR-PRISMA-001',
    shortMessage: 'Systematic review protocol/reporting metadata incomplete',
    detailedMessage: 'Systematic review workflows must document PRISMA items, protocol registration, eligibility criteria, screening, extraction, and risk-of-bias plan.',
    category: 'methodological',
    severity: 'high',
    suggestedCorrection: 'Link PRISMA 2020, PRISMA-P, PROSPERO, AMSTAR 2, and ROBIS references as applicable.',
    supportingReferenceIds: ['PRISMA_2020', 'PRISMA_P_2015', 'PROSPERO_REGISTRATION', 'AMSTAR2', 'ROBIS'],
    applicableStudyTypes: ['systematic_review'],
  },
  {
    id: 'EC-014',
    code: 'MA-HET-001',
    shortMessage: 'Meta-analysis heterogeneity/publication-bias plan missing',
    detailedMessage: 'Meta-analysis protocols must pre-specify pooling model, heterogeneity statistics, sensitivity analysis, and publication-bias assessment.',
    category: 'statistical',
    severity: 'high',
    suggestedCorrection: 'Document fixed/random-effects model choice, I-squared, tau-squared, funnel plot, Egger/Begg tests, and sensitivity analysis.',
    supportingReferenceIds: ['PRISMA_2020', 'PRISMA_NMA', 'MOOSE_GUIDELINES', 'PUBLICATION_BIAS_TESTS'],
    applicableStudyTypes: ['meta_analysis'],
  },
];
