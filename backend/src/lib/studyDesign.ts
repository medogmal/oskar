...
export type BlindedParty = 'patient' | 'researcher' | 'assessor' | 'statistician';
...
const blindedPartyOrder: BlindedParty[] = ['patient', 'researcher', 'assessor', 'statistician'];
...
const normalizeBlindedParties = (parties?: BlindedParty[]) =>
  blindedPartyOrder.filter((party) => parties?.includes(party));
